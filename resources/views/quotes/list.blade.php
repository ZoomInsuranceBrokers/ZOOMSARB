                ...
                <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                    @if(!$quote->is_final_submit)
                        ...
                        <a href="{{ route('quotes.download', $quote->id) }}"
                           onclick="event.stopPropagation(); return handlePdfDownload(event, {{ $quote->id }}, false);"
                    ...
                    @endif
                    <form method="POST" action="{{ route('quotes.destroy', $quote->id) }}" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this quote?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                onclick="event.stopPropagation();"
